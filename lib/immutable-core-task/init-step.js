...
function initStep (args) {
    ...
    if (args.check && defined(spec.check)) {
        ...
        step.check = this.initStep({map: true, name: 'step check', spec: spec.check})
    }
    // add error method
    if (args.error) {
        // add error method
        ...
        // set ignore error flag for step
        if (defined(spec.ignoreError)) {
            // require boolean
            this.assert(_.isBoolean(spec.ignoreError), 127)
            // set flag
            step.ignoreError = spec.ignoreError
        }
        // use global default setting if defined
        else if (defined(this.ignoreError)) {
            step.ignoreError = this.ignoreError
        }
    }
    // add input map
