...
async function completeError () {
    ...
    if (this.task.hasReverse && !this.status.completeError) {
        // set flag to re-run completeError when reverse complete
        // this may be overriden by completeFailure if reverse fails
        this.status.completeError = true
        // run reverse
        await this.runReverse()
    }
    // complete task
    else {
...
async function completeFailure () {
    ...
    if (this.task.hasReverse && !this.status.completeError && !this.status.completeFailure) {
...
async function run () {
    ...
    for (await this.nextStep(); defined(this.status) && defined(this.status.step); await this.nextStep()) {
        // throw error if running the same step twice in a row to catch
