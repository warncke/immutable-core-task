...
function nextStep () {
    ...
    this.step = this.task.steps[stepNum]
    // require step
    this.assert(defined(this.step), 200, `step ${stepNum} not defined`)
    // working in reverse
    if (this.status.reverse) {
        this.status.error ? this.nextStepReverseError() : this.nextStepReverse()
    }
    // working forward
    else {
        this.status.error ? this.nextStepForwardError() : this.nextStepForward()
    }
}
