...
function nextStep () {
    ...
    var error = defined(this.status[`${lastSubStep}Error`])
    // get method name for stub step
    var subStepMethodName = _.upperFirst(lastSubStep)
    // add error if sub step had error
    if (error) {
        subStepMethodName = `${subStepMethodName}Error`
    }
    ...
    return this.status.reverse
        // run reverse step based on last sub step
        ? this[`nextStepReverse${subStepMethodName}`]()
        // run forward step based on last sub step
        : this[`nextStepForward${subStepMethodName}`]()
}

/**
 * @function nextStepForwardNone
 *
...
function nextStepForwardNone () {
    ...
    this.status.step = this.task.steps[stepNum]
    // require step
    this.assert(defined(this.step), 200, `step ${stepNum} not defined`)
    // start with method sub-step
    this.status.subStep = 'method'
}

/**
 * @function nextStepForwardMethod
 *
...
/**
 * @function nextStepForwardMethodError
 *
...
function nextStepForwardMethodError () {
    ...
    if (this.shouldRetry()) {
        // if method has check then run that
        if (defined(step.check)) {
            this.status.subStep = 'check'
        }
        // otherwise re-run method
        else {
            return
        }
    }
...
async function run () {
    ...
    while (true) {
        // get next step
        this.nextStep()
        // only continue if step defined
        if (!defined(this.step)) {
            break
        }
        try {
            // run step method
            var result = await this.runStep()
            // handle success
...
async function runError (error) {
    ...
    // if method error is already defined then this is an error on 
    // add error to status
    this.status.error = _.pick(error, ['code', 'data', 'message', 'stack'])
    // check if retry should be done for error
    if (this.shouldRetry()) {
        // do retry
        await this.retry()
    }
    // if step has an error handler this should be run after any retries
    else if (this.shouldError()) {
        // continue to nextStep which will call error method
        return
    }
    // check if there are any reverse methods that should be run
...
async function runStep () {
    ...
    var method = this.task.getMethod(this.step.method)
    // get args - apply input map if defined
    var args = defined(this.step.input)
        ? this.getInput(this.data, this.step.input)
        : this.data
    // run method
    var result = await (method)(args)
    // apply output map if defined
    if (defined(this.step.output)) {
        result = this.getOutput(result, this.step.output)
    }
 ...
 *
 * return true if error handler should be called
 *
 * @returns {boolean}
 */
function shouldError () {
    // if step does not have error handler then cannot call
    if (!defined(this.step.error)) {
        return false
    }
 ...
 *
 * return true if there are any steps with reverse
 *
 * @returns {boolean}
 */
function shouldReverse () {
